...
const BookingModal = ({ isOpen, onClose, categories = serviceCategories }) => {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${!selectedCategory ? 'bg-gray-100 cursor-not-allowed opacity-60' : ''
                  }`}
              >
                <option value="">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button
                ...
                className={`flex-1 ${selectedService
...
const HomePage = () => {
  ...
  const formatPrice = (p) => {
    if (!p) return "";
    const cleanPrice = String(p).replace(/[^0-9.]/g, '');
    const num = Number(cleanPrice);
    if (!isNaN(num) && cleanPrice.length > 0) {
      // Return with ₹ safely if it didn't have it natively
      const hasSymbol = String(p).includes('₹');
      const prefix = hasSymbol ? '₹' : '';
      return `${prefix}${new Intl.NumberFormat('en-IN').format(num)}`;
    }
    return p;
  };
  ...
  const activeCategories = services.length > 0 ? services.map((cat, index) => {
    const originalCat = serviceCategories.find(c => c.title === cat.title || c.id === cat.id);
    return {
      id: cat.id || originalCat?.id || `category-${index}`,
      title: cat.title || originalCat?.title || "Other Services",
      description: cat.description || originalCat?.description || "Professional legal and financial services.",
      icon: cat.icon || originalCat?.icon || FileText,
      color: cat.color || originalCat?.color || "from-gray-500 to-gray-600",
      services: cat.services || []
    };
  }) : serviceCategories;

  const allServicesList = activeCategories.flatMap(cat => 
    cat.services.map(s => ({
      ...s,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              ...
              growth. Click on any service to explore detailed features.
            </p>
          </div>
          {allServicesList.length > 0 && <HorizontalSlider
            items={allServicesList}
            slidesPerView={3}
            renderCard={(service) => {
              const IconComponent = service.categoryIcon || FileText;
              return (
                ...
                <CardContent className="p-6 flex flex-col justify-between h-full">
                  <div>
                    <div className="flex items-start justify-between mb-4">
                      <div className={`w-12 h-12 bg-gradient-to-r ${service.categoryColor || "from-blue-500 to-blue-600"} rounded-xl flex items-center justify-center shadow-lg`}>
                    ...
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center gap-3 text-sm">
                        <span className="font-bold text-green-600 text-lg">
                          {!String(service.price).includes('₹') && '₹'}{formatPrice(service.price)}
                        </span>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-2xl font-bold text-green-600">
                        {!String(bundle.price).includes('₹') && '₹'}{formatPrice(bundle.price)}
                      </span>
                      {bundle.originalPrice && (
                        <span className="text-sm text-gray-400 line-through">
                          {!String(bundle.originalPrice).includes('₹') && '₹'}{formatPrice(bundle.originalPrice)}
                        </span>
                      )}
                    </div>
      ...
      {/* Booking Modal */}
      <BookingModal isOpen={isBookingModalOpen} onClose={closeBookingModal} categories={activeCategories} />
